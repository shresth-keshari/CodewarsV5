...
def initialize(my_tower, opp_tower, my_troops, opp_troops):
	...
	"Balloon": ["Minion", "Wizard", "Musketeer", "Archer", "Dragon"],
	"Skeleton": ["Valkyrie", "Dragon", "Wizard", "Skeleton", "Minion"],
	"Giant": ["Skeleton", "Barbarian", "Minion", "Wizard", "Prince"], 
	"Prince": ["Skeleton", "Barbarian", "Prince", "Minion", "Wizard"],
	"Wizard": ["Knight", "Prince", "Valkyrie", "Wizard"], 
	"Barbarian": ["Valkyrie", "Wizard", "Skeleton", "Prince", "Dragon", "Barbarian"],
	"Knight": ["Skeleton", "Barbarian", "Valkyrie", "Prince", "Knight", "Minion", "Dragon", "Wizard", "Musketeer"],
	"Valkyrie": ["Knight", "Prince", "Wizard", "Valkyrie", "Minion", "Musketeer"],
	"Dragon": ["Wizard", "Dragon"],
	"Minion": ["Dragon", "Wizard", "Minion", "Archer", "Musketeer"],
	"Musketeer": ["Minion", "Knight", "Skeleton", "Barbarian", "Valkyrie", "Musketeer", "Dragon", "Prince", "Wizard"],
	"Archer": ["Skeleton", "Prince", "Wizard", "Barbarian", "Knight", "Valkyrie", "Dragon", "Minion", "Musketeer", "Archer"],             
	}
	...
	if not emergency and not attacking:
		...
		for troop in threatening_ordered_arriving_troops:
			if troop.name in distance_damage_troops or troop.name in ["Giant", "Balloon"]:
				...
				return position
		position = ((temp_troop.position[0]) / 2, (3 * 9.375 + temp_troop.position[1]) / 4)
		return position
	if attacking:
		for troop in threatening_ordered_arriving_troops:
			if my_troop_to_deploy in distance_damage_troops:
...
def defend(my_troops, opp_troops, my_tower, length_of_team_signal, distance_damage_troops, indexes_of_troops_in_recommended_counters, max_value_for_each_troop, value_for_each_counter, recommended_counters, my_troops_order, my_troops_list, my_elixir):
	...
	# for main_troop in temp_opp_troops:
	# 	if Utils.calculate_distance(my_tower, main_troop) - my_tower.size - main_troop.size < 45:
	# 		if main_troop.position[0] < 0:
	# 			for troop in temp_opp_troops:
	# 				if Utils.calculate_distance(troop, main_troop) - troop.size - main_troop.size < 7:
	# 					temp_opp_troops.remove(troop)
	...
	# 		if main_troop.position[0] < 0:
	# 			for troop in temp_opp_troops:
	# 				if Utils.calculate_distance(troop, main_troop) - troop.size - main_troop.size < 6:
	# 					temp_opp_troops.remove(troop)
	# 					left_troops_to_consider.append(troop)
	# 		if main_troop.position[0] > 0:
	# 			for troop in temp_opp_troops:
