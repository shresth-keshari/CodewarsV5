...
def update_opponent_elixir(elixir_index, troop_deployed=None, local_time=None):
	...
	if opponent_elixir < 0:
		opponent_elixir = 4
		change_team_signal(elixir_index, "4")
		change_team_signal(elixir_index + 1, "0")
		change_team_signal(elixir_index + 2, "0")
	else:
	...
	if attacking:
		for troop in threatening_ordered_arriving_troops:
			...
			if troop.name in distance_damage_troops or troop.name in ["Giant", "Balloon"]:
				...
				return position
		position = (temp_troop.position[0], 50)
		return position
	for troop in threatening_ordered_arriving_troops:
		if my_troop_to_deploy in distance_damage_troops:
	...
	if good_enough_databasing_done:
		if not len(left_troops_to_consider):
			...
			for troop in my_troops_order[:4]:
				...
				index_of_troops_in_recommended_counters = find_index(indexes_of_troops_in_recommended_counters, troop)
				max_value = max_value_for_each_troop[index_of_troops_in_recommended_counters]
				score = 0
...
def logic(arena_data: dict):
	...
	if local_time == 0:
		...
		# 		(attack_troops_to_deploy, position_of_attack_troops_to_deploy) = attack(length_of_team_signal, good_enough_databasing_done, opponent_troops_list, opponent_troops_order, my_troops, opp_troops, no_sole_attack, distance_damage_troops, indexes_of_troops_in_recommended_counters, max_value_for_each_troop, value_for_each_counter, recommended_counters, my_troops_order, my_troops_list, my_elixir)
	...
	elif local_time == 1 and team_signal[length_of_team_signal - 29] != "0":
		...
		if to_deploy:
			...
			if not int(team_signal[length_of_team_signal - 29]):
				...
				if attack_troops_to_deploy:
					if len(attack_troops_to_deploy) >= 2 or (len(attack_troops_to_deploy) == 1 and (left_attacked or right_attacked)) or ["Giant"] in attack_troops_to_deploy or ["Balloon"] in attack_troops_to_deploy:
						counter = 0
						for troop_to_deploy in attack_troops_to_deploy:
							my_elixir -= elixir(troop_to_deploy)
							if my_elixir >= 2:
	...
	end_time = time.time()
	print(f"Time for per frame database processing: {round(time_in_processing * 1000000, 0)} microseconds")
	print(f"Time for per frame strategy processing: {round((end_time - start_time) * 1000000, 0)} microseconds")
	print(f"Total Time: {round((time_in_processing + end_time - start_time) * 1000000)} microseconds")