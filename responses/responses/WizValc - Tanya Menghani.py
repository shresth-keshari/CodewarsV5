...
def logic(arena_data: dict):
    ...
    enemy_x_positions = [troop.position[0] for troop in opp_troops]
    if not enemy_x_positions:
        deploy_x = 0  # Default to center if no enemies
    elif all(x > 0 for x in enemy_x_positions):
        if all(x > 13 for x in enemy_x_positions):
           deploy_x = 19.5
        if all(x < 13 for x in enemy_x_positions):
            deploy_x = 6.5
        else:
        ...
        if all(x < -12 for x in enemy_x_positions):
            deploy_x = -18.5
        elif all(x > -12 for x in enemy_x_positions):
            deploy_x = -6.5
        else:
            deploy_x = -12
    else:
        deploy_x = 0  # Enemies are on both sides, deploy in the middle
...
.helper_function import Troops, Utils
